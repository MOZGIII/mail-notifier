...
pub struct OAuth2Context {
    /// OAuth 2 clietns.
    pub oauth2_clients: std::collections::HashMap<String, OAuth2Client>,
...
) -> Result<std::collections::HashMap<String, OAuth2Client>, OAuth2ClientError> {
        ...
        config_core::Auth::OAuth2Session(oauth2) => {
            ...
            let session_manager = oauth2_session::Manager {
                expiration_immenance_tolerance: std::time::Duration::from_secs(
                    oauth2.expiration_immenance_tolerance_secs.unwrap_or(60),
                ),
